...
model_path = "/home/josh/Documents/ibots/2_12_26_full_integer_quant_edgetpu.tflite"
...
# Set up distance finding stuff
distance_to_ball = 3

def wait_for_n():
    print("Loop paused. Press 'n' to continue...")
    with keyboard.Events() as events:
        for event in events:
            # Check if the event is a key press (not release)
            if isinstance(event, keyboard.Events.Press):
                # Use hasattr to safely check for alphanumeric characters
                if hasattr(event.key, 'char') and event.key.char == 'n':
                    break

with open("tw.txt", "a") as tw_file, open("th.txt", "a") as th_file:

    # Run regular inferencing
    while cap.isOpened():
        for _ in range(5):
        ...
        if success:
            ...
            for b in boxes:
                ...
                tw_file.write(f"{distance_to_ball}, {tw}\n")
                th_file.write(f"{distance_to_ball}, {th}\n")
