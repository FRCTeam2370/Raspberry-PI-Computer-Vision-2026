...
CAMERA_FOV_HORIZONTAL = 0.7772799999999804   # CAMERA_FOV_HORIZONTAL should be in radians
CAMERA_FOV_VERTICAL = 0.5725600000000031   # CAMERA_FOV_VERTICAL should be in radians
CAMERA_HORIZONTAL_OFFSET = 0.03718741
CAMERA_VERTICAL_OFFSET = 0.02260995 
CAMERA_INPUT_INDEX = 3
...
FIELD_MAX_Y = 100#16.54
FIELD_MAX_X = 100#8.07
...
def coral_worker():
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            intake_down = table.getEntry("intakeDown").getBoolean(intake_down)
            ...
            for b in boxes:
                ...
                pitch_radian = pitch_radian + CAMERA_VERTICAL_OFFSET
                yaw_radian = yaw_radian + CAMERA_HORIZONTAL_OFFSET
                ...
                if distance_height > distance_width: distance = round(distance_height, 2)
                else: distance = round(distance_width, 2)
                ...
                if pitch_radian < PITCH_WHERE_TARGET_IS_NOT_ON_GROUND:
                    keep = False
                # Check if fuel is within field coords
                if ball_position[0] > FIELD_MAX_X:
                    print("Fuel out of bounds...")
                    keep = False
                if ball_position[1] > FIELD_MAX_Y:
                    print("Fuel out of bounds...")
                    keep = False
            ...
            if intake_down:
                ...
                table.putBoolean("coral_connected", True)
                table.putNumberArray("ball_position_x", ball_positions_x)
                table.putNumberArray("ball_position_y", ball_positions_y)
            else:
                table.putBoolean("coral_connected", True)
                table.putNumberArray("ball_position_x", [0])
                table.putNumberArray("ball_position_y", [0])
        ...
        #   print("Exception: " + str(e))
            # Coral is not connected
        #  table.putBoolean("coral_connected", False)
        # table.putNumberArray("ball_position_x", [0])
            #table.putNumberArray("ball_position_y", [0])
...
while True:
    ...
    print("Worker Done")
    if p.exitcode != 0:
        # Coral is disconnected 
        table.putBoolean("coral_connected", False)
        table.putNumberArray("ball_position_x", [0])
        table.putNumberArray("ball_position_y", [0])
        print("Coral Worker Process Failed...")
